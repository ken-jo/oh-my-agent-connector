...
*Đừng học Claude Code. Cứ dùng OMAC.*

[Bắt đầu nhanh](#bắt-đầu-nhanh) • [Tài liệu](https://yeachan-heo.github.io/oh-my-agent-connector-website) • [Tham chiếu CLI](https://yeachan-heo.github.io/oh-my-agent-connector-website/docs/#cli-reference) • [Quy trình](https://yeachan-heo.github.io/oh-my-agent-connector-website/docs/#workflows) • [Hướng dẫn di chuyển](docs/MIGRATION.md) • [Discord](https://discord.gg/sj4exxQ9v)
...
Nếu bạn chạy OMAC qua `omac --plugin-dir <path>` hoặc `claude --plugin-dir <path>`, hãy thêm `--plugin-dir-mode` vào `omac setup` (hoặc xuất `OMAC_PLUGIN_ROOT` trước) để tránh trùng lặp các kỹ năng/tác nhân mà plugin đã cung cấp trong thời gian chạy. Để xem ma trận quyết định đầy đủ và tất cả các cờ có sẵn, hãy xem [phần Plugin directory flags trong REFERENCE.md](./docs/REFERENCE.md#plugin-directory-flags).
...
Bắt đầu từ **v4.1.7**, **Team** là bề mặt điều phối chuẩn trong OMAC. Các điểm vào cũ như **swarm** và **ultrapilot** vẫn được hỗ trợ, nhưng giờ đây chúng **được chuyển sang Team ở tầng bên dưới**.
...
> Nếu teams bị tắt, OMAC sẽ cảnh báo và chuyển sang chế độ thực thi không dùng team khi có thể.

### Công nhân CLI tmux — Codex & Gemini (v4.4.0+)

**v4.4.0 xóa các máy chủ MCP Codex/Gemini** (nhà cung cấp `x`, `g`). Dùng `/omac-teams` để khởi động tiến trình CLI thực sự trong các pane tmux phân chia:
...
|-------|---------|----------|
| `/omac-teams N:codex` | N pane Codex CLI | Xem xét code, phân tích bảo mật, kiến trúc |
| `/omac-teams N:gemini` | N pane Gemini CLI | Thiết kế UI/UX, tài liệu, tác vụ ngữ cảnh lớn |
| `/omac-teams N:claude` | N pane Claude CLI | Tác vụ chung qua Claude CLI trong tmux |
...
> **Lưu ý: Tên package** — Dự án được xây dựng thương hiệu là **oh-my-agent-connector** (repo, plugin, commands), nhưng package npm được phát hành dưới tên [`oh-my-agent-connector`](https://www.npmjs.com/package/oh-my-agent-connector). Nếu bạn cài công cụ CLI qua npm/bun, hãy dùng `npm install -g oh-my-agent-connector`.
...
> **Lưu ý:** Nếu tự động cập nhật marketplace chưa được bật, bạn cần chạy `/plugin marketplace update omac` thủ công để đồng bộ phiên bản mới nhất trước khi chạy setup.
...
## Vì sao chọn oh-my-agent-connector?
...
Nhiều chiến lược cho nhiều tình huống — từ điều phối dựa trên Team đến refactor tiết kiệm token. [Tìm hiểu thêm →](https://yeachan-heo.github.io/oh-my-agent-connector-website/docs/#execution-modes)
...
| **omac-teams** | Công nhân CLI tmux — tiến trình `claude`/`codex`/`gemini` thực trong pane chia | Tác vụ Codex/Gemini CLI; tạo theo yêu cầu, tắt khi xong |
...
Bạn muốn đóng góp cho OMAC? Xem [CONTRIBUTING.md](./CONTRIBUTING.md) để biết hướng dẫn nhà phát triển đầy đủ, bao gồm cách fork, thiết lập checkout cục bộ, liên kết nó làm plugin hoạt động, chạy các bài kiểm tra và gửi PR.

<!-- TODO(i18n): verify translation -->

### Kỹ năng Tùy chỉnh

Học một lần, tái sử dụng mãi mãi. OMAC trích xuất kiến thức gỡ lỗi thực chiến thành các tệp kỹ năng di động, tự động tiêm vào khi phù hợp.

| | Phạm vi Dự án | Phạm vi Người dùng |
|---|---|---|
| **Đường dẫn** | `.omac/skills/` | `~/.omac/skills/` |
...
| `omac-teams` | Công nhân CLI tmux (codex/gemini/claude) | `/omac-teams 2:codex "security review"` |
...
**Thiết lập thủ công:** tạo `~/.claude/omac_config.openclaw.json`:
...
| Biến | Mô tả |
|------|-------|
| `OMAC_OPENCLAW=1` | Bật OpenClaw |
| `OMAC_OPENCLAW_DEBUG=1` | Bật ghi log gỡ lỗi |
| `OMAC_OPENCLAW_CONFIG=/path/to/config.json` | Thay đổi đường dẫn file cấu hình |
...
- **[Tham chiếu CLI](https://yeachan-heo.github.io/oh-my-agent-connector-website/docs/#cli-reference)** - Tất cả lệnh, cờ và công cụ `omac`
- **[Hướng dẫn thông báo](https://yeachan-heo.github.io/oh-my-agent-connector-website/docs/#notifications)** - Thiết lập Discord, Telegram, Slack và webhook
- **[Quy trình khuyến nghị](https://yeachan-heo.github.io/oh-my-agent-connector-website/docs/#workflows)** - Chuỗi skill đã qua thực chiến cho các tác vụ phổ biến
- **[Ghi chú phát hành](https://yeachan-heo.github.io/oh-my-agent-connector-website/docs/#release-notes)** - Có gì mới trong mỗi phiên bản
- **[Website](https://yeachan-heo.github.io/oh-my-agent-connector-website)** - Hướng dẫn tương tác và ví dụ
...
OMAC có thể tùy chọn điều phối các nhà cung cấp AI bên ngoài để đối chiếu chéo và nhất quán thiết kế. Đây **không bắt buộc** — OMAC vẫn hoạt động đầy đủ mà không cần chúng.
