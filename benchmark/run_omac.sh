...
# Verify OMAC is built
if [ ! -d "$PROJECT_ROOT/dist" ] || [ ! -f "$PROJECT_ROOT/dist/index.js" ]; then
    log_error "oh-my-agent-connector is not built. Run: npm run build"
    exit 1
fi

log_info "=========================================="
log_info "Running OMAC-Enhanced Benchmark"
log_info "=========================================="
log_info "Mode: $RUN_MODE (with oh-my-agent-connector orchestration)"
...
log_step "Starting OMAC-enhanced benchmark run..."
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    log_info "  2. Compare results: python3 compare_results.py --vanilla predictions/vanilla --omac predictions/omac"
    log_info ""
else
    log_error "=========================================="
