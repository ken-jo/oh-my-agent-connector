...
Automated benchmark comparison between vanilla Claude Code and OMAC-enhanced Claude Code.
...
- Tests both vanilla and OMAC modes
...
- Standard Claude Code without OMAC
...
### run_omac.sh
Run OMAC-enhanced benchmark:
- Claude Code with oh-my-agent-connector orchestration
- Saves predictions to `predictions/omac/`
- Logs to `logs/omac_*.log`
...
**Examples:**
```bash
./run_omac.sh                    # Full benchmark
./run_omac.sh --limit 100        # First 100 instances
```
...
- `--skip-omac` - Skip OMAC benchmark run
...
./run_full_comparison.sh --skip-vanilla     # Only run OMAC (reuse vanilla results)
```
...
├── run_omac.sh                 # Run OMAC benchmark
...
│   └── omac/                  # OMAC predictions
├── logs/
│   ├── vanilla_*.log         # Vanilla run logs
│   └── omac_*.log            # OMAC run logs
├── results/
│   ├── vanilla_results.json  # Vanilla evaluation
│   ├── omac_results.json      # OMAC evaluation
...
4. **Review Results**:
   ...
   - Inspect predictions in `predictions/vanilla/` and `predictions/omac/`
   - Review logs in `logs/`
...
./run_omac.sh --model claude-opus-4-6-20260205
```
...
2. **Parallel Runs**: Don't run vanilla and OMAC in parallel (share API rate limits)
