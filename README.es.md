...
[Comenzar](#inicio-rápido) • [Documentación](https://yeachan-heo.github.io/oh-my-agent-connector-website) • [Referencia CLI](https://yeachan-heo.github.io/oh-my-agent-connector-website/docs/#cli-reference) • [Flujos de Trabajo](https://yeachan-heo.github.io/oh-my-agent-connector-website/docs/#workflows) • [Guía de Migración](docs/MIGRATION.md) • [Discord](https://discord.gg/sj4exxQ9v)
...
A partir de **v4.1.7**, **Team** es la superficie canónica de orquestación en OMAC. Los puntos de entrada legados como **swarm** y **ultrapilot** siguen siendo compatibles, pero ahora **enrutan a Team internamente**.
...
> Si los equipos están desactivados, OMAC te avisará y hará fallback a ejecución sin Team cuando sea posible.

### Trabajadores CLI tmux — Codex & Gemini (v4.4.0+)

**v4.4.0 elimina los servidores MCP de Codex/Gemini** (proveedores `x`, `g`). Usa `/omac-teams` para lanzar procesos CLI reales en paneles divididos de tmux:
...
|-------|---------|----------|
| `/omac-teams N:codex` | N paneles Codex CLI | Revisión de código, análisis de seguridad, arquitectura |
| `/omac-teams N:gemini` | N paneles Gemini CLI | Diseño UI/UX, docs, tareas de gran contexto |
| `/omac-teams N:claude` | N paneles Claude CLI | Tareas generales via Claude CLI en tmux |
...
> **Nota: Nombre del paquete** — El proyecto usa la marca **oh-my-agent-connector** (repositorio, plugin, comandos), pero el paquete npm se publica como [`oh-my-agent-connector`](https://www.npmjs.com/package/oh-my-agent-connector). Si instalas las herramientas CLI via npm/bun, usa `npm install -g oh-my-agent-connector`.
...
> **Nota:** Si la actualizacion automatica del marketplace no esta activada, debes ejecutar manualmente `/plugin marketplace update omac` para sincronizar la ultima version antes de ejecutar el setup.
...
### Modos de Ejecución
Múltiples estrategias para diferentes casos de uso - desde construcciones completamente autónomas hasta refactorización eficiente en tokens. [Aprende más →](https://yeachan-heo.github.io/oh-my-agent-connector-website/docs/#execution-modes)
...
| **omac-teams** | Trabajadores CLI tmux | Tareas Codex/Gemini CLI; se inician bajo demanda, terminan al completar |
...
Aprende una vez, reutiliza para siempre. OMAC extrae conocimiento valioso de depuración en archivos de habilidades portátiles que se inyectan automáticamente cuando son relevantes.

| | Alcance de Proyecto | Alcance de Usuario |
|---|---|---|
| **Ruta** | `.omac/skills/` | `~/.omac/skills/` |
...
| `omac-teams` | Trabajadores CLI tmux (codex/gemini/claude) | `/omac-teams 2:codex "security review"` |
...
```bash
omac wait          # Verificar estado, obtener orientación
omac wait --start  # Habilitar demonio de reanudación automática
omac wait --stop   # Deshabilitar demonio
```
...
**Configuración manual:** crea `~/.claude/omac_config.openclaw.json`:
...
| `OMAC_OPENCLAW_DEBUG=1` | Habilitar registro de depuración |
| `OMAC_OPENCLAW_CONFIG=/path/to/config.json` | Ruta alternativa del archivo de configuración |
...
- **[Referencia CLI](https://yeachan-heo.github.io/oh-my-agent-connector-website/docs/#cli-reference)** - Todos los comandos, flags y herramientas de `omac`
- **[Guía de Notificaciones](https://yeachan-heo.github.io/oh-my-agent-connector-website/docs/#notifications)** - Configuración de Discord, Telegram, Slack y webhooks
- **[Flujos de Trabajo Recomendados](https://yeachan-heo.github.io/oh-my-agent-connector-website/docs/#workflows)** - Cadenas de habilidades probadas para tareas comunes
- **[Notas de Versión](https://yeachan-heo.github.io/oh-my-agent-connector-website/docs/#release-notes)** - Novedades en cada versión
- **[Sitio Web](https://yeachan-heo.github.io/oh-my-agent-connector-website)** - Guías interactivas y ejemplos
...
OMAC puede opcionalmente orquestar proveedores de IA externos para validación cruzada y consistencia de diseño. **No son necesarios** — OMAC funciona completamente sin ellos.
