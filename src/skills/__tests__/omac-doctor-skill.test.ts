...
describe('omac-doctor skill (issue #2254)', () => {
  it('documents CLAUDE.md OMAC version drift check against cached plugin version', () => {
    const skillPath = join(process.cwd(), 'skills', 'omac-doctor', 'SKILL.md');
    const content = readFileSync(skillPath, 'utf8');

    expect(content).toContain('CLAUDE.md OMAC version:');
    expect(content).toContain('OMAC version source:');
    ...
    expect(content).toContain('If `CLAUDE.md OMAC version` != `Latest cached plugin version`: WARN - version drift detected');
  });
});


describe('omac-doctor skill Ralph Ruby dependency check (issue #2969)', () => {
...
describe('omac-doctor skill package version diagnostic (issue #2981)', () => {
  it('checks the canonical published npm package for latest version', () => {
    const skillPath = join(process.cwd(), 'skills', 'omac-doctor', 'SKILL.md');
    const content = readFileSync(skillPath, 'utf8');

    expect(content).toContain('npm view oh-my-agent-connector version');
    expect(content.match(/npm view /g)).toHaveLength(1);
  });
});
