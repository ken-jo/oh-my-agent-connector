 ...
 *
 * Affected files: skills/omac-setup/SKILL.md, skills/hud/SKILL.md
 */
...
describe('MINGW64 escape safety: no "!" in node -e inline scripts (issue #729)', () => {
  ...
  describe('skills/omac-setup (SKILL.md + phases)', () => {
    const setupDir = join(REPO_ROOT, 'skills', 'omac-setup');
    const filesToScan = [
    ...
    it('has no "!" in any node -e script body (MINGW64 safe)', () => {
      const violations = findBangViolations(allScripts, 'omac-setup/*');
      if (violations.length > 0) {
        expect.fail(
          'Found "!" in node -e scripts (breaks MINGW64/Git Bash):\n' +
          violations.map(v => `  • ${v}`).join('\n')
  ...
  describe('specific regressions (issue #729)', () => {
    ...
    it('hud SKILL.md keeps Unix statusLine guidance portable while preserving Windows-safe paths', () => {
      const content = readFileSync(join(REPO_ROOT, 'skills', 'hud', 'SKILL.md'), 'utf-8');
      expect(content).toContain('"command": "node ${CLAUDE_CONFIG_DIR:-$HOME/.claude}/hud/omac-hud.mjs"');
      expect(content).toContain('"command": "node C:/Users/username/.claude/hud/omac-hud.mjs"');
      expect(content).not.toContain('"command": "node /home/username/.claude/hud/omac-hud.mjs"');
    ...
    it('hud SKILL.md cleanup step removes only the legacy HUD wrapper filename', () => {
      ...
      const cleanupLine = content
        .split('\n')
        .find(l => l.includes('Removed legacy omac-hud.js') && l.startsWith('node -e'));

      expect(cleanupLine).toBeDefined();
      expect(cleanupLine).toContain("t=p.join(d,'hud','omac-hud.js')");
      expect(cleanupLine).not.toContain("t=p.join(d,'hud','omac-hud.mjs')");
    });

    it("omac-setup version-detect script uses v==='' not !v", () => {
      const setupDir = join(REPO_ROOT, 'skills', 'omac-setup');
      const files = [
        join(setupDir, 'SKILL.md'),
        ...readdirSync(join(setupDir, 'phases')).map(f => join(setupDir, 'phases', f)),
    ...
    it('omac-setup extracts CLAUDE.md version from OMAC marker', () => {
      const setupDir = join(REPO_ROOT, 'skills', 'omac-setup');
      const files = [
        join(setupDir, 'SKILL.md'),
        ...readdirSync(join(setupDir, 'phases')).map(f => join(setupDir, 'phases', f)),
      ...
      const combined = files.map(f => readFileSync(f, 'utf-8')).join('\n');
      expect(combined).toContain("grep -m1 'OMAC:VERSION:'");
      expect(combined).not.toContain('grep -m1 "^# oh-my-agent-connector"');
    });

    it('omac-setup SKILL.md explicitly tells the agent to execute immediately', () => {
      const content = readFileSync(
        join(REPO_ROOT, 'skills', 'omac-setup', 'SKILL.md'),
        'utf-8'
      );
    ...
    it('omac-setup phase 2 delegates HUD setup instead of inlining statusLine formatting', () => {
      const content = readFileSync(
