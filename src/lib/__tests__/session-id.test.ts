...
const ORIGINAL_ENV = process.env.OMAC_SESSION_ID;

afterEach(() => {
  if (ORIGINAL_ENV === undefined) delete process.env.OMAC_SESSION_ID;
  else process.env.OMAC_SESSION_ID = ORIGINAL_ENV;
});

describe('resolveSessionId', () => {
  describe('context = cli', () => {
    it('returns env when only env is set', () => {
      process.env.OMAC_SESSION_ID = 'env-sid';
      expect(resolveSessionId({ context: 'cli' })).toBe('env-sid');
    });

    it('returns payload when only payload is set', () => {
      delete process.env.OMAC_SESSION_ID;
