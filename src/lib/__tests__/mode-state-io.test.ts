...
describe('mode-state-io', () => {
  ...
  describe('writeModeState', () => {
    it('should write state with _meta containing written_at and mode', () => {
      ...
      const filePath = join(tempDir, '.omac', 'state', 'ralph-state.json');
    ...
    it('should write session-scoped state when sessionId is provided', () => {
      ...
      const filePath = join(tempDir, '.omac', 'state', 'sessions', 'pid-123-1000', 'ultrawork-state.json');
      expect(existsSync(filePath)).toBe(true);
    ...
    it('should create parent directories as needed', () => {
      ...
      expect(result).toBe(true);
      expect(existsSync(join(tempDir, '.omac', 'state'))).toBe(true);
    });
    ...
    it('should resolve writes to the git worktree root when called from a subdirectory', () => {
      ...
      expect(result).toBe(true);
      expect(existsSync(join(tempDir, '.omac', 'state', 'autopilot-state.json'))).toBe(true);
      expect(existsSync(join(nestedDir, '.omac', 'state', 'autopilot-state.json'))).toBe(false);
    });
    ...
    it('should include sessionId in _meta when sessionId is provided', () => {
      ...
      const filePath = join(tempDir, '.omac', 'state', 'sessions', 'pid-session-42', 'ralph-state.json');
      expect(existsSync(filePath)).toBe(true);
  ...
  describe('readModeState', () => {
    ...
    it('should read from session path when sessionId is provided', () => {
      const sessionDir = join(tempDir, '.omac', 'state', 'sessions', 'pid-999-2000');
  ...
  describe('clearModeStateFile', () => {
    it('should clear state from the git worktree root when given a subdirectory', () => {
      ...
      expect(existsSync(join(nestedDir, '.omac', 'state', 'ralph-state.json'))).toBe(false);
    });
    ...
    it('should delete session-scoped state file', () => {
      const sessionDir = join(tempDir, '.omac', 'state', 'sessions', 'pid-100-500');
      mkdirSync(sessionDir, { recursive: true });
    ...
    it('should perform ghost-legacy cleanup for files with matching session_id', () => {
      ...
      const sessionDir = join(tempDir, '.omac', 'state', 'sessions', 'pid-200-600');
      mkdirSync(sessionDir, { recursive: true });
    ...
    it('should clean up legacy root-level mode files for the matching session', () => {
      const legacyRootPath = join(tempDir, '.omac', 'ralph-state.json');
      mkdirSync(join(tempDir, '.omac'), { recursive: true });
      writeFileSync(
        legacyRootPath,
    ...
    it('should remove all session-scoped files when no session_id is provided', () => {
      const sessionAPath = join(tempDir, '.omac', 'state', 'sessions', 'session-a', 'ralph-state.json');
      const sessionBPath = join(tempDir, '.omac', 'state', 'sessions', 'session-b', 'ralph-state.json');
      mkdirSync(join(tempDir, '.omac', 'state', 'sessions', 'session-a'), { recursive: true });
      mkdirSync(join(tempDir, '.omac', 'state', 'sessions', 'session-b'), { recursive: true });
