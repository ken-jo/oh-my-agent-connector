...
/**
 * Map from user-facing OMAC_DISABLE_TOOLS group names to ToolCategory values.
...
/**
 * Parse OMAC_DISABLE_TOOLS env var value into a Set of disabled ToolCategory values.
 *
 * Accepts a comma-separated list of group names (case-insensitive).
 * Unknown names are silently ignored.
 *
 * @param envValue - The env var value to parse. Defaults to process.env.OMAC_DISABLE_TOOLS.
 * @returns Set of ToolCategory values that should be disabled.
 *
 * @example
 * // OMAC_DISABLE_TOOLS=lsp,python-repl,project-memory
...
export function parseDisabledGroups(envValue?: string): Set<ToolCategory> {
  ...
  const value = envValue ?? process.env.OMAC_DISABLE_TOOLS;
...
const interopToolsEnabled = process.env.OMAC_INTEROP_TOOLS_ENABLED === '1';
...
// Read OMAC_DISABLE_TOOLS once at startup and filter tools accordingly
...
/**
 * In-process MCP server exposing all OMAC custom tools
 *
 * Tools will be available as mcp__t__<tool_name>.
 * Tools in disabled groups (via OMAC_DISABLE_TOOLS) are excluded at startup.
 */
export const omacToolsServer = createSdkMcpServer({
  name: "t",
  version: "1.0.0",
  tools: sdkTools
});
 ...
 * Only includes tools that are enabled (not disabled via OMAC_DISABLE_TOOLS).
 */
export const omacToolNames = enabledTools.map(t => `mcp__t__${t.name}`);

// Build a map from MCP tool name to category for efficient lookup
// Built from allTools so getOmacToolNames() category filtering works correctly
 ...
 */
export function getOmacToolNames(options?: ToolNameFilterOptions): string[] {
  return filterToolNames(omacToolNames, toolCategoryMap, options);
}
