...
describe('shutdownTeamV2 detached worktree cleanup', () => {
  ...
  beforeEach(() => {
    ...
    repoDir = mkdtempSync(join(tmpdir(), 'omac-runtime-v2-shutdown-'));
  ...
  it('removes dormant team-created worktrees during normal shutdown', async () => {
    ...
    const teamRoot = join(repoDir, '.omac', 'state', 'team', teamName);
  ...
  it('keeps team state when config is missing but worktree root AGENTS backup exists', async () => {
    ...
    writeFileSync(backupPath, JSON.stringify({
      worktreePath: join(repoDir, '.omac', 'team', teamName, 'worktrees', 'worker-1'),
      hadOriginal: true,
  ...
  it('uses the canonical team state root in worktree shutdown ack instructions', async () => {
    ...
    expect(inbox).toContain('$OMAC_TEAM_STATE_ROOT/workers/worker-wt/shutdown-ack.json');
    expect(inbox).not.toContain(`Write your ack to: .omac/state/team/${teamName}`);
  });
