...
describe('team api dispatch-aware messaging', () => {
  ...
  beforeEach(async () => {
    cwd = await mkdtemp(join(tmpdir(), 'omac-team-api-dispatch-'));
  ...
  it('persists leader-fixed messages and leaves a durable pending dispatch request when the leader pane is absent', async () => {
    ...
    const mailboxPath = join(cwd, '.omac', 'state', 'team', teamName, 'mailbox', 'leader-fixed.json');
  ...
  it('updates delivered and notified markers on the same canonical mailbox record', async () => {
    ...
    const mailboxPath = join(cwd, '.omac', 'state', 'team', teamName, 'mailbox', 'worker-1.json');
    const mailbox = JSON.parse(await readFile(mailboxPath, 'utf-8')) as {
  ...
  it('uses OMAC_TEAM_STATE_ROOT placeholder in mailbox triggers for worktree-backed workers', async () => {
    const configPath = join(cwd, '.omac', 'state', 'team', teamName, 'config.json');
    await writeFile(configPath, JSON.stringify({
      ...
      workers: [{
        ...
        role: 'executor',
        assigned_tasks: [],
        worktree_path: join(cwd, '.omac', 'worktrees', teamName, 'worker-1'),
      }],
      created_at: '2026-03-06T00:00:00.000Z',
      next_task_id: 2,
    }, null, 2));
    ...
    const requests = await listDispatchRequests(teamName, cwd, { kind: 'mailbox', to_worker: 'worker-1' });
    expect(requests).toHaveLength(1);
    expect(requests[0]?.trigger_message).toContain('$OMAC_TEAM_STATE_ROOT/mailbox/worker-1.json');
