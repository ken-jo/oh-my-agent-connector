...
describe('scaleUp launch config', () => {
  ...
  beforeEach(async () => {
    cwd = await mkdtemp(join(tmpdir(), 'omac-scaling-launch-config-'));
  ...
  it.each([
    ...
    expect(tmuxSessionMocks.buildWorkerStartCommand).toHaveBeenCalledWith(expect.objectContaining({
      ...
      envVars: expect.objectContaining({
        OMAC_TEAM_WORKER: 'demo-team/worker-1',
        OMAC_TEAM_NAME: 'demo-team',
        OMAC_WORKER_AGENT_TYPE: agentType,
        OMAC_TEAM_STATE_ROOT: `${resolve(cwd)}/.omac/state/team/demo-team`,
        OMAC_TEAM_LEADER_CWD: resolve(cwd),
      }),
    }));
  });
  ...
  it('rolls back a pending worktree when scale-up fails before worker config is saved', async () => {
    ...
    gitWorktreeMocks.ensureWorkerWorktree.mockReturnValue({
      path: join(resolve(cwd), '.omac', 'team', 'demo-team', 'worktrees', 'worker-1'),
      branch: 'omac-team/demo-team/worker-1',
      workerName: 'worker-1',
      teamName: 'demo-team',
