 ...
 * detected" in a multi-repo .omac-workspace layout.
 *
 * In that layout the shared .omac root lives at the workspace anchor (the parent
 * of each sub-repo), so paths built with getOmacRoot(subRepo) resolve ABOVE the
 ...
 *
 * Setup mirrors the reviewer's reproduction: parent/.omac-workspace + parent/api.
 */
...
describe('multi-repo workspace team writes', () => {
  ...
  beforeEach(() => {
    ...
    parent = mkdtempSync(join(tmpdir(), 'omac-multirepo-team-'));
  ...
  const sharedOmac = () => join(parent, '.omac');

  it('audit log writes land under the shared .omac, not the sub-repo', () => {
    ...
    const logPath = join(sharedOmac(), 'logs', `team-bridge-${teamName}.jsonl`);
    expect(existsSync(logPath)).toBe(true);
    // Must NOT have written into the sub-repo's local .omac.
    expect(existsSync(join(api, '.omac', 'logs', `team-bridge-${teamName}.jsonl`))).toBe(false);
    expect(readAuditLog(api, teamName)).toHaveLength(1);
  });

  it('usage records write under the shared .omac without traversal error', () => {
    ...
    const logPath = join(sharedOmac(), 'logs', `team-usage-${teamName}.jsonl`);
    expect(existsSync(logPath)).toBe(true);
  });

  it('restart state writes under the shared .omac without traversal error', () => {
    expect(() => recordRestart(api, teamName, 'worker1')).not.toThrow();

    const statePath = join(sharedOmac(), 'state', 'team-bridge', teamName, 'worker1.restart.json');
  ...
  it('worktree creation + metadata + AGENTS overlay work from the sub-repo', () => {
    ...
    // Worktree and metadata live under the shared workspace .omac, above the repo.
    expect(info.path.startsWith(join(sharedOmac(), 'team'))).toBe(true);
    expect(existsSync(info.path)).toBe(true);
    expect(existsSync(join(sharedOmac(), 'state', 'team', teamName, 'worktrees.json'))).toBe(true);
