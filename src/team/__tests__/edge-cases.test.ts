...
describe('task-file-ops edge cases', () => {
  beforeEach(() => {
    TASK_TEST_CWD = join(realpathSync(tmpdir()), `omac-edge-tasks-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    TASKS_DIR = join(TASK_TEST_CWD, '.omac', 'state', 'team', EDGE_TEAM_TASKS, 'tasks');
    mkdirSync(TASKS_DIR, { recursive: true });
  });
...
describe('heartbeat edge cases', () => {
  ...
  describe('readHeartbeat with corrupt JSON file', () => {
    it('returns null for corrupt heartbeat file', () => {
      const dir = join(HB_DIR, '.omac', 'state', 'team-bridge', 'test-team');
...
describe('tmux-session edge cases', () => {
  ...
  describe('sessionName total length', () => {
    it('each part is truncated to 50 chars independently', () => {
      ...
      // 'omac-team-' + 50 chars + '-' + 50 chars = 111 total
      expect(result.length).toBe(111);
      expect(result).toBe(`omac-team-${'a'.repeat(50)}-${'a'.repeat(50)}`);
    });
  });
...
describe('team-registration edge cases', () => {
  beforeEach(() => {
    ...
    mkdirSync(join(REG_DIR, '.omac', 'state'), { recursive: true });
  ...
  describe('readProbeResult with corrupt JSON', () => {
    it('returns null for malformed probe result file', () => {
      const probePath = join(REG_DIR, '.omac', 'state', 'config-probe-result.json');
  ...
  describe('listMcpWorkers with malformed shadow registry', () => {
    it('returns empty when shadow registry is corrupt JSON', () => {
      const shadowPath = join(REG_DIR, '.omac', 'state', 'team-mcp-workers.json');
