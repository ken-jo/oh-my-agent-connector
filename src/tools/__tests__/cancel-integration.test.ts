...
describe('cancel-integration', () => {
  ...
  describe('1. Single-session cancel with ghost-legacy cleanup', () => {
    it('should clear session files AND ghost legacy files when session_id provided', async () => {
      ...
      // Create ghost legacy file at .omac/state/ralph-state.json with matching session
      writeFileSync(
      ...
      writeFileSync(
        join(TEST_DIR, '.omac', 'state', 'ultrawork-state.json'),
      ...
      expect(existsSync(join(TEST_DIR, '.omac', 'state', 'ralph-state.json'))).toBe(false);
      expect(existsSync(join(TEST_DIR, '.omac', 'state', 'ultrawork-state.json'))).toBe(false);
    ...
    it('should NOT delete legacy autopilot ghost file owned by a different session via top-level session_id', async () => {
      ...
      writeFileSync(
        join(TEST_DIR, '.omac', 'state', 'autopilot-state.json'),
      ...
      expect(existsSync(join(TEST_DIR, '.omac', 'state', 'autopilot-state.json'))).toBe(true);
  ...
  describe('2. Force cancel (no session_id)', () => {
    it('should clear ALL files across all sessions plus legacy', async () => {
      ...
      for (const sid of sessions) {
        const sessionDir = join(TEST_DIR, '.omac', 'state', 'sessions', sid);
        mkdirSync(sessionDir, { recursive: true });
        writeFileSync(
          join(sessionDir, 'ralph-state.json'),
      ...
      for (const sid of sessions) {
        const sessionPath = join(TEST_DIR, '.omac', 'state', 'sessions', sid, 'ralph-state.json');
  ...
  describe('4. Stale cleanup', () => {
    it('should detect and deactivate state files with old _meta.updatedAt', () => {
      ...
      const stateFile = join(TEST_DIR, '.omac', 'state', 'ralph-state.json');
    ...
    it('should NOT deactivate state files with recent _meta.updatedAt', () => {
      ...
      const stateFile = join(TEST_DIR, '.omac', 'state', 'ultrawork-state.json');
      writeFileSync(stateFile, JSON.stringify({
        active: true,
        _meta: {
  ...
  describe('5. Team cancel', () => {
    it('should clear team state at both session and legacy paths', async () => {
      ...
      const sessionDir = join(TEST_DIR, '.omac', 'state', 'sessions', sessionId);
      mkdirSync(sessionDir, { recursive: true });
      const runtimeTeamDir = join(TEST_DIR, '.omac', 'state', 'team', 'demo-team');
      ...
      writeFileSync(
        join(TEST_DIR, '.omac', 'state', 'team-state.json'),
        JSON.stringify({ active: true, phase: 'team-exec', team_name: 'demo-team', _meta: { sessionId } })
      );

      writeFileSync(
        join(TEST_DIR, '.omac', 'state', 'mission-state.json'),
        JSON.stringify({
      ...
      expect(existsSync(join(TEST_DIR, '.omac', 'state', 'team-state.json'))).toBe(false);
      expect(existsSync(runtimeTeamDir)).toBe(false);

      const missionState = JSON.parse(readFileSync(join(TEST_DIR, '.omac', 'state', 'mission-state.json'), 'utf-8'));
    ...
    it('should clear team state at session path while preserving unrelated legacy', async () => {
      ...
      expect(existsSync(join(TEST_DIR, '.omac', 'state', 'team-state.json'))).toBe(true);
    });

    it('should remove all team runtime roots on broad team clear', async () => {
      mkdirSync(join(TEST_DIR, '.omac', 'state', 'team', 'alpha-team'), { recursive: true });
      mkdirSync(join(TEST_DIR, '.omac', 'state', 'team', 'beta-team'), { recursive: true });
      writeFileSync(
        join(TEST_DIR, '.omac', 'state', 'mission-state.json'),
        JSON.stringify({
          updatedAt: new Date().toISOString(),
      ...
      expect(existsSync(join(TEST_DIR, '.omac', 'state', 'team'))).toBe(false);
