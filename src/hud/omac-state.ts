/**
 * OMAC HUD - State Readers
 *
 * Read ralph, ultrawork, and PRD state from existing OMAC files.
 ...
 * 1. Session-scoped paths (.omac/state/sessions/{id}/{filename}) - newest first
 * 2. Standard path (.omac/state/{filename})
 * 3. Legacy path (.omac/{filename})
 *
...
function resolveStatePath(directory: string, filename: string, sessionId?: string): string | null {
  const omacRoot = getOmacRoot(directory);

  if (sessionId) {
    const sessionPath = join(omacRoot, 'state', 'sessions', sessionId, filename);
  ...
  // Check standard path
  const newPath = join(omacRoot, 'state', filename);
  ...
  // Check legacy path
  const legacyPath = join(omacRoot, filename);
 ...
 * Checks only local .omac/state location.
 */
 ...
 * Checks both root prd.json and .omac/prd.json.
 */
export function readPrdStateForHud(directory: string): PrdStateForHud | null {
  ...
  if (!existsSync(prdPath)) {
    // Check .omac
    prdPath = join(getOmacRoot(directory), 'prd.json');
