/**
 * OMAC Orchestrator Hook
 *
 * Enforces orchestrator behavior - delegation over direct implementation.
 * When an orchestrator agent tries to directly modify files outside .omac/,
 * this hook injects reminders to delegate to subagents instead.
 *
 * Adapted from oh-my-opencode's omac-orchestrator hook for shell-based hooks.
 */
 ...
 * Checks: .omac/config.json → [$CLAUDE_CONFIG_DIR|~/.claude]/.omac-config.json → default (warn)
 */
function getEnforcementLevel(directory: string): EnforcementLevel {
  ...
  const localConfig = path.join(getOmacRoot(directory), 'config.json');
  const globalConfig = path.join(getClaudeConfigDir(), '.omac-config.json');
...
/**
 * Create omac orchestrator hook handlers
 */
export function createOmacOrchestratorHook(directory: string) {
  return {
    /**
