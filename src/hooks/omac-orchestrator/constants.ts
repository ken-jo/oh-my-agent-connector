/**
 * OMAC Orchestrator Constants
 *
 * Message templates and configuration for orchestrator behavior enforcement.
 *
 * Adapted from oh-my-opencode's omac-orchestrator hook.
 */

export const HOOK_NAME = 'omac-orchestrator';

/** @deprecated Use ALLOWED_PATH_PATTERNS instead. Legacy single prefix. */
export const ALLOWED_PATH_PREFIX = '.omac/';
...
You just performed direct file modifications outside \`.omac/\`.
...
You should NOT:
- Write code directly (except for \`.omac/\` files like plans and notepads)
- Make direct file edits outside \`.omac/\`
...
You (coordinator) are attempting to directly modify a file outside \`.omac/\`.
...
- Files inside \`.omac/\` (plans, notepads, drafts)
...
- Creating new files outside \`.omac/\`
...
- Use the notepad at .omac/notepads/{PLAN_NAME}/ to record learnings
