...
describe('team cli help text surfaces', () => {
  it('team.ts usage includes legacy and api surfaces', () => {
    ...
    expect(source).toContain('omac team resume <team_name>');
    expect(source).toContain('omac team shutdown <team_name>');
    expect(source).toContain('omac team api <operation>');
    expect(source).toContain('omac team [ralph] <N:agent-type[:role]>');
  });
