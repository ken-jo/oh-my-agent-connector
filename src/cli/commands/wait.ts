 ...
 *
 * Design Philosophy (aligned with oh-my-agent-connector values):
 * - Zero learning curve: `omac wait` just works
 * - Smart defaults: Auto-detects tmux and daemon status
 * - Minimal commands: Most users only need `omac wait`
 *
 * Commands:
 *   omac wait               - Smart command: shows status, offers to start daemon if needed
 *   omac wait status        - Show current rate limit and daemon status
 *   omac wait daemon start  - Start the background daemon
 *   omac wait daemon stop   - Stop the daemon
 *   omac wait detect        - Scan for blocked Claude Code sessions
 */
...
export async function waitCommand(options: WaitOptions): Promise<void> {
  ...
  if (rateLimitStatus.isLimited) {
    ...
    if (!tmuxAvailable) {
      ...
      console.log(chalk.white('   Run: ') + chalk.green('omac wait --start'));
      console.log(chalk.gray('   (or: omac wait daemon start)\n'));
    } else {
    ...
    if (daemonRunning) {
      ...
      console.log(chalk.gray('Stop with: omac wait --stop\n'));
    }
  }
}
  ...
  if (action === 'start') {
    if (options.foreground) {
      ...
      if (result.success) {
        ...
        console.log(chalk.gray('\nUse "omac wait status" to check daemon status'));
        console.log(chalk.gray('Use "omac wait daemon stop" to stop the daemon'));
      } else {
...
export async function waitDetectCommand(options: WaitDetectOptions): Promise<void> {
  ...
  if (result.state?.blockedPanes && result.state.blockedPanes.length > 0) {
    ...
    console.log(chalk.gray('  omac wait daemon start'));
  }
