...
describe('hud omac state session scoping', () => {
  ...
  function createWorktree(): string {
    const dir = mkdtempSync(join(tmpdir(), 'omac-hud-state-'));
    tempDirs.push(dir);
    return dir;
  }
  ...
  it('keeps backward-compatible newest-session fallback when sessionId is omitted', () => {
    ...
    const omacRoot = join(worktree, '.omac');
    const older = Date.now() - 60_000;
    const newer = Date.now();

    writeJson(join(omacRoot, 'state', 'sessions', 'session-a', 'ralph-state.json'), {
      active: true,
      iteration: 1,
      max_iterations: 5,
      current_story_id: 'story-a',
    }, older);
    writeJson(join(omacRoot, 'state', 'sessions', 'session-b', 'ralph-state.json'), {
      active: true,
      iteration: 4,
      max_iterations: 7,
  ...
  it('does not leak to other sessions or fallback files when a session-scoped file is missing', () => {
    ...
    writeJson(join(omacRoot, 'state', 'sessions', 'session-b', 'autopilot-state.json'), {
      active: true,
      phase: 'execution',
      iteration: 3,
      max_iterations: 10,
    ...
    writeJson(join(omacRoot, 'state', 'autopilot-state.json'), {
      active: true,
      phase: 'qa',
      iteration: 8,
      max_iterations: 10,
  ...
  it('applies session scoping to combined mode helpers', () => {
    ...
    writeJson(join(omacRoot, 'state', 'sessions', 'session-b', 'ultrawork-state.json'), {
      active: true,
