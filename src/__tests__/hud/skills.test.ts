...
describe('renderSkills', () => {
  ...
  describe('namespaced skill names', () => {
    it('displays only last segment for namespaced skills (oh-my-agent-connector:plan)', () => {
      const lastSkill: SkillInvocation = { name: 'oh-my-agent-connector:plan', timestamp: new Date() };
      const result = renderSkills(inactiveUltrawork, inactiveRalph, lastSkill);
      expect(result).toContain('skill:plan');
      expect(result).not.toContain('oh-my-agent-connector');
    });

    it('displays only last segment for namespaced skills with args', () => {
      const lastSkill: SkillInvocation = { name: 'oh-my-agent-connector:autopilot', args: 'build app', timestamp: new Date() };
      const result = renderSkills(inactiveUltrawork, inactiveRalph, lastSkill);
