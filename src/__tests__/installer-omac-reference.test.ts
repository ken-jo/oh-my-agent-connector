...
function writeEnabledPluginSettings(claudeConfigDir: string): void {
  writeFileSync(
    ...
    JSON.stringify({ plugins: ['oh-my-agent-connector'] }, null, 2)
  );
}
...
function writeMinimallyCompletePluginPayload(pluginRoot: string): void {
  ...
  writeFileSync(join(pluginRoot, 'commands', 'omac-setup.md'), 'Read skills/omac-setup/SKILL.md.\n');
  ...
  writeFileSync(join(pluginRoot, 'package.json'), JSON.stringify({ name: 'oh-my-agent-connector', version: '4.10.2' }, null, 2));
}
...
describe('installer bundled + standalone skill sync', () => {
  ...
  beforeEach(() => {
    tempRoot = mkdtempSync(join(tmpdir(), 'omac-installer-omac-reference-'));
  ...
  it('installs standalone slash skills into ~/.claude/skills during legacy install', async () => {
    ...
    expect(result.installedSkills).toEqual(expect.arrayContaining([
      ...
      'team/SKILL.md',
      'ultrawork/SKILL.md',
      'omac-reference/SKILL.md',
      'omac-plan/SKILL.md',
    ]));

    for (const skillName of ['autopilot', 'ralph', 'ralplan', 'team', 'ultrawork', 'omac-reference', 'omac-plan']) {
  ...
  it('installs bundled skills when no enabled OMAC plugin is configured', async () => {
    const pluginRoot = join(tempRoot, 'plugin-cache', 'oh-my-agent-connector', '4.10.2');
    mkdirSync(join(pluginRoot, 'skills', 'ralph'), { recursive: true });
    writeFileSync(join(pluginRoot, 'skills', 'ralph', 'SKILL.md'), 'name: ralph\n');
    ...
    expect(result.installedSkills).toContain('omac-reference/SKILL.md');
    expect(result.installedSkills).toContain('ralph/SKILL.md');
    expect(result.installedSkills).toContain('omac-plan/SKILL.md');

    for (const skillName of ['omac-reference', 'ralph', 'team']) {
      const installedSkillPath = join(claudeConfigDir, 'skills', skillName, 'SKILL.md');
      expect(existsSync(installedSkillPath)).toBe(true);
      expect(readFileSync(installedSkillPath, 'utf-8')).toContain(`name: ${skillName}`);
    }

    expect(existsSync(join(claudeConfigDir, 'skills', 'omac-setup', 'phases', '04-welcome.md'))).toBe(true);
  });
