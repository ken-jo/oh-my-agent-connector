...
describe('Keyword Detector', () => {
  ...
  describe('detectKeywordsWithType', () => {
    ...
    it('should detect cancel keyword', () => {
      const detected = detectKeywordsWithType('cancelomac this task');
      expect(detected).toHaveLength(1);
      expect(detected[0].type).toBe('cancel');
      expect(detected[0].keyword).toBe('cancelomac');
    });

    it('should detect cancel keyword variations', () => {
      const cancelTerms = ['cancelomac', 'stopomac'];
  ...
  describe('getPrimaryKeyword', () => {
    ...
    it('should give cancel highest priority', () => {
      const primary = getPrimaryKeyword('stopomac searching for files');
      expect(primary).not.toBeNull();
      expect(primary!.type).toBe('cancel');
    });

    it('should give cancel priority over analyze', () => {
      const primary = getPrimaryKeyword('cancelomac this investigation');
      expect(primary).not.toBeNull();
      expect(primary!.type).toBe('cancel');
    });

    it('should prioritize cancel over all other keywords', () => {
      const primary = getPrimaryKeyword('stopomac ultrawork and search');
      expect(primary).not.toBeNull();
      expect(primary!.type).toBe('cancel');
    });
    ...
    it('should not detect ralph in /oh-my-agent-connector:ralph-init', () => {
      const primary = getPrimaryKeyword('/oh-my-agent-connector:ralph-init "my project"');
...
describe('Team staged workflow integration', () => {
  ...
  beforeEach(() => {
    testDir = join(tmpdir(), `omac-team-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    mkdirSync(join(testDir, '.omac', 'state', 'sessions', sessionId), { recursive: true });
  ...
  it('restores active Team stage on session-start', async () => {
    writeFileSync(
      join(testDir, '.omac', 'state', 'sessions', sessionId, 'team-state.json'),
      JSON.stringify({
        active: true,
  ...
  it('compacts OMAC-style root AGENTS guidance on session-start without dropping key sections', async () => {
    const agentsContent = `# oh-my-agent-connector - Intelligent Multi-Agent Orchestration
...
- preserve verification
  ...
  it('keeps session-start under budget when only a tiny omission remainder remains', async () => {
    writeFileSync(
      join(testDir, '.omac', 'state', 'sessions', sessionId, 'ultrawork-state.json'),
      JSON.stringify({
        active: true,
        session_id: sessionId,
...
<skills>${'- drop skills\n'.repeat(500)}</skills>`
  ...
  it('trips Team continuation circuit breaker after max stop reinforcements', async () => {
    ...
    writeFileSync(
      join(testDir, '.omac', 'state', 'sessions', sessionId, 'team-pipeline-stop-breaker.json'),
  ...
  it('bypasses autopilot continuation when transcript context is critically exhausted', async () => {
    ...
    writeFileSync(
      join(testDir, '.omac', 'state', 'sessions', sessionId, 'autopilot-state.json'),
      JSON.stringify({
        active: true,
        phase: 'execution',
        session_id: sessionId,
...
describe('Persistent-mode reply cleanup behavior', () => {
  ...
  beforeEach(() => {
    testDir = join(tmpdir(), `omac-reply-cleanup-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    tempHome = join(tmpdir(), `omac-reply-home-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('does not remove reply-session registry on idle Stop/persistent-mode', async () => {
    const registryPath = join(homedir(), '.omac', 'state', 'reply-session-registry.jsonl');
    mkdirSync(join(homedir(), '.omac', 'state'), { recursive: true });
    writeFileSync(
      registryPath,
      `${JSON.stringify({
        platform: 'telegram',
...
describe('Mutual Exclusion - UltraQA and Ralph', () => {
  ...
  beforeEach(() => {
    ...
    testDir = join(tmpdir(), `omac-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    mkdirSync(testDir, { recursive: true });
  ...
  describe('isUltraQAActive', () => {
    ...
    it('should return true when ultraqa is active', () => {
      const stateFile = join(testDir, '.omac', 'state', 'ultraqa-state.json');
  ...
  describe('isRalphLoopActive', () => {
    ...
    it('should return true when ralph is active', () => {
      const stateFile = join(testDir, '.omac', 'state', 'ralph-state.json');
      writeFileSync(stateFile, JSON.stringify({ active: true }));
  ...
  describe('UltraQA mutual exclusion', () => {
    it('should fail to start UltraQA when Ralph is active', () => {
      ...
      const sessionDir = join(testDir, '.omac', 'state', 'sessions', 'test-session');
    ...
    it('should succeed starting UltraQA when ralph state exists but inactive', () => {
      const ralphStateFile = join(testDir, '.omac', 'state', 'ralph-state.json');
  ...
  describe('Ralph mutual exclusion', () => {
    ...
    it('should succeed starting Ralph when ultraqa state exists but inactive', () => {
      const ultraqaStateFile = join(testDir, '.omac', 'state', 'ultraqa-state.json');
