...
describe('auto-update reconciliation', () => {
  beforeEach(() => {
    ...
    mockedReaddirSync.mockImplementation((path: Parameters<typeof readdirSync>[0], options?: Parameters<typeof readdirSync>[1]) => {
      ...
      if (normalized.endsWith('/commands')) {
        return options && typeof options === 'object' && 'withFileTypes' in options
          ? [{ name: 'omac-setup.md', isFile: () => true, isDirectory: () => false }] as any
          : ['omac-setup.md'] as any;
      }
    ...
    mockedReadFileSync.mockImplementation((path: Parameters<typeof readFileSync>[0]) => {
      ...
      if (normalized.endsWith('/.claude-plugin/plugin.json')) {
        return JSON.stringify({ name: 'oh-my-agent-connector', commands: './commands/', skills: ['./skills/plan/'] });
      }
      if (normalized.includes('.omac-version.json')) {
  ...
  afterEach(() => {
    vi.unstubAllGlobals();
    delete process.env.OMAC_UPDATE_RECONCILE;
  ...
  it('reconciles runtime state without re-injecting settings hooks', () => {
    ...
    expect(mockedMkdirSync).toHaveBeenCalledWith('/tmp/omac-test-hooks', { recursive: true });
  ...
  it('syncs active plugin cache roots and logs when copy occurs', () => {
    ...
    const activeRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omac', 'oh-my-agent-connector', '4.1.5');
    ...
    mockedReadFileSync.mockImplementation((path: Parameters<typeof readFileSync>[0]) => {
      ...
      if (normalized.endsWith('/plugins/installed_plugins.json')) {
        return JSON.stringify({
          plugins: {
            'oh-my-agent-connector': [{ installPath: activeRoot }],
          },
        });
      }
      return '';
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('[omac update] Synced plugin cache');
  });



  it('fails reconciliation when active plugin cache repair reports validation errors', () => {
    const activeRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omac', 'oh-my-agent-connector', '4.14.1');
  ...
  it('skips plugin cache sync silently when no active plugin roots exist', () => {
    ...
    expect(consoleLogSpy).not.toHaveBeenCalledWith('[omac update] Synced plugin cache');
  });


  it('updates installed_plugins.json to the new cache version after plugin cache sync succeeds', () => {
    const cacheRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omac', 'oh-my-agent-connector');
    ...
    mockedReadFileSync.mockImplementation((path: Parameters<typeof readFileSync>[0]) => {
      ...
      if (normalized === '/usr/lib/node_modules/oh-my-agent-connector/package.json') {
      ...
      if (normalized.endsWith('/plugins/installed_plugins.json')) {
        return JSON.stringify({
          version: 2,
          plugins: {
            'oh-my-agent-connector@omac': [{ installPath: oldRoot, version: '4.14.0', enabled: true }],
          },
        });
      }
      return '';
    ...
    mockedExistsSync.mockImplementation((path: Parameters<typeof existsSync>[0]) => {
      const normalized = String(path).replace(/\\/g, '/');
      return normalized.endsWith('/plugins/cache/omac/oh-my-agent-connector')
        || normalized.endsWith('/plugins/installed_plugins.json')
        || normalized.startsWith(join(cacheRoot, '4.14.1').replace(/\\/g, '/'))
        || normalized.startsWith('/usr/lib/node_modules/oh-my-agent-connector');
    });
  ...
  it('preserves Windows-style installPath separators when rewriting installed_plugins.json', () => {
    const cacheRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omac', 'oh-my-agent-connector');
    const oldWindowsRoot = 'C:\\Users\\bellman\\.claude\\plugins\\cache\\omac\\oh-my-agent-connector\\4.14.0';
    const newWindowsRoot = 'C:\\Users\\bellman\\.claude\\plugins\\cache\\omac\\oh-my-agent-connector\\4.14.1';
    ...
    mockedReadFileSync.mockImplementation((path: Parameters<typeof readFileSync>[0]) => {
      ...
      if (normalized === 'C:/Users/bellman/AppData/Roaming/npm/node_modules/oh-my-agent-connector/package.json') {
        return JSON.stringify({ version: '4.14.1' });
      }
      if (normalized.endsWith('/plugins/installed_plugins.json')) {
        return JSON.stringify({
          version: 2,
          plugins: {
            'oh-my-agent-connector@omac': [{ installPath: oldWindowsRoot, version: '4.14.0' }],
          },
        });
      }
      return '';
    ...
    mockedExistsSync.mockImplementation((path: Parameters<typeof existsSync>[0]) => {
      ...
      return normalized === cacheRoot.replace(/\\/g, '/')
        || normalized.endsWith('/plugins/installed_plugins.json')
        || normalized.startsWith(join(cacheRoot, '4.14.1').replace(/\\/g, '/'))
        || normalized.startsWith('C:/Users/bellman/AppData/Roaming/npm/node_modules/oh-my-agent-connector');
    });
    ...
    expect(result.errors).toEqual([]);
    const written = String(mockedWriteFileSync.mock.calls.find(([path]) => String(path).includes('installed_plugins.json.tmp-'))?.[1]);
    expect(JSON.parse(written).plugins['oh-my-agent-connector@omac'][0]).toMatchObject({
      version: '4.14.1',
    ...
    mockedReadFileSync.mockImplementation((path: Parameters<typeof readFileSync>[0]) => {
      ...
      if (normalized.endsWith('/plugins/installed_plugins.json')) {
        return JSON.stringify({
          version: 2,
          plugins: {
            'oh-my-agent-connector@omac': [{ installPath: join(cacheRoot, '4.14.0'), version: '4.14.0' }],
          },
        });
      }
      return '';
  ...
  it('syncs the plugin cache directory when cache root exists', () => {
    ...
    mockedExistsSync.mockImplementation((path: Parameters<typeof existsSync>[0]) => {
      ...
      if (normalized.startsWith('/usr/lib/node_modules/oh-my-agent-connector/')) {
        return normalized.endsWith('/dist')
          ...
          || normalized.endsWith('/commands/omac-setup.md')
    ...
    expect(mockedCpSync).toHaveBeenCalledWith(
      '/usr/lib/node_modules/oh-my-agent-connector/dist',
      `${versionedCacheRoot}/dist`,
      expect.objectContaining({ recursive: true, force: true }),
    );
    expect(mockedCpSync).toHaveBeenCalledWith(
      '/usr/lib/node_modules/oh-my-agent-connector/package.json',
      `${versionedCacheRoot}/package.json`,
      expect.objectContaining({ recursive: true, force: true }),
    );
    expect(consoleLogSpy).toHaveBeenCalledWith('[omac update] Plugin cache synced');
  });
  ...
  it('handles plugin cache sync errors non-fatally', () => {
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      `[omac update] Plugin cache sync warning: Failed to sync dist to ${versionedCacheRoot}: copy failed`,
    );
  });
  ...
  it('only blocks standalone update inside an active plugin session', () => {
    ...
    process.env.CLAUDE_PLUGIN_ROOT = '/tmp/.claude/plugins/cache/omac/oh-my-agent-connector/4.1.5';
    expect(shouldBlockStandaloneUpdateInCurrentSession()).toBe(false);
  ...
  it('dedupes plugin roots and ignores missing targets during sync', () => {
    const consoleLogSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    const activeRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omac', 'oh-my-agent-connector', '4.1.5');
    const staleRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omac', 'oh-my-agent-connector', '4.1.4');
  ...
  it('allows standalone update when CLAUDE_PLUGIN_ROOT is inherited without an active Claude session', async () => {
    const pluginRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omac', 'oh-my-agent-connector', '4.1.5');
    const cacheRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'omac', 'oh-my-agent-connector');
    ...
    mockedExecSync.mockImplementation((command: string) => {
      if (command === 'npm install -g oh-my-agent-connector@latest') {
        return '';
      }
      if (command === 'npm root -g') {
        return '/usr/lib/node_modules\n';
    ...
    expect(result.success).toBe(true);
    expect(mockedExecSync).toHaveBeenCalledWith('npm install -g oh-my-agent-connector@latest', expect.any(Object));
  });
  ...
  it('restores global Claude Code when npm removes an existing global install during update', async () => {
    ...
    mockedExistsSync.mockImplementation((path: Parameters<typeof existsSync>[0]) => {
      ...
      if (normalized.endsWith('/plugins/marketplaces/omac')) {
        return false;
      }
      if (normalized.endsWith('/plugins/cache/omac/oh-my-agent-connector')) {
        return false;
      }
      return true;
    });
    ...
    try {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith('[omac update] Restoring global @anthropic-ai/claude-code@1.2.3 after npm update...');
      expect(consoleLogSpy).toHaveBeenCalledWith('[omac update] Restored global @anthropic-ai/claude-code');
    } finally {
  ...
  it('skips marketplace auto-sync when the marketplace clone has local modifications', async () => {
    ...
    expect(mockedExecFileSync).toHaveBeenCalledWith(
      'git',
      ['-C', expect.stringContaining('/plugins/marketplaces/omac'), 'status', '--porcelain', '--untracked-files=normal'],
      expect.any(Object)
    );
  ...
  it('skips marketplace auto-sync when the marketplace clone has local commits', async () => {
    ...
    expect(mockedExecFileSync).toHaveBeenCalledWith(
      'git',
      ['-C', expect.stringContaining('/plugins/marketplaces/omac'), 'rev-list', '--left-right', '--count', 'HEAD...origin/main'],
      expect.any(Object)
    );
    expect(mockedExecFileSync).not.toHaveBeenCalledWith(
      'git',
  ...
  it('fast-forwards a clean marketplace clone when origin/main is ahead', async () => {
    ...
    expect(mockedExecFileSync).toHaveBeenCalledWith(
      'git',
      ['-C', expect.stringContaining('/plugins/marketplaces/omac'), 'merge', '--ff-only', 'origin/main'],
      expect.any(Object)
    );
    expect(mockedExecFileSync).not.toHaveBeenCalledWith(
      'git',
  ...
  it('re-execs with omac.cmd on Windows and persists metadata after reconciliation', async () => {
    mockPlatform('win32');
    ...
    expect(result.success).toBe(true);
    expect(mockedExecSync).toHaveBeenCalledWith('npm install -g oh-my-agent-connector@latest', expect.objectContaining({
      windowsHide: true,
    }));
    expect(mockedExecFileSync).toHaveBeenCalledWith('where.exe', ['omac.cmd'], expect.objectContaining({
      encoding: 'utf-8',
      stdio: 'pipe',
      timeout: 5000,
      windowsHide: true,
    }));
    expect(mockedExecFileSync).toHaveBeenCalledWith('C:\\Users\\bellman\\AppData\\Roaming\\npm\\omac.cmd', ['update-reconcile'], expect.objectContaining({
      encoding: 'utf-8',
      stdio: 'pipe',
      timeout: 60000,
      shell: true,
      windowsHide: true,
      env: expect.objectContaining({ OMAC_UPDATE_RECONCILE: '1' }),
    }));
    expect(mockedWriteFileSync).toHaveBeenCalledWith(expect.stringContaining('.omac-version.json'), expect.stringContaining('"version": "4.1.6"'));
  });
