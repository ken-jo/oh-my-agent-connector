...
function stateFile(cwd: string, sessionId: string, name: string) {
  return join(cwd, '.omac', 'state', 'sessions', sessionId, `${name}-state.json`);
}
...
describe('keyword-detector.mjs — pasted system-echo re-entry guard', () => {
  ...
  it('does NOT re-activate ralph when the prompt is a Stop hook feedback block', () => {
    ...
    const prompt = [
      ...
      'When FULLY complete (after Architect verification), run /oh-my-agent-connector:cancel ...',
  ...
  // `When FULLY complete`, and `run /oh-my-agent-connector:cancel`, which meant
