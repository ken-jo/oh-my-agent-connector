...
```
/oh-my-agent-connector:sciomac <goal>                    # Standard research with user checkpoints
/oh-my-agent-connector:sciomac AUTO: <goal>              # Fully autonomous until complete
/oh-my-agent-connector:sciomac status                    # Check current research session status
/oh-my-agent-connector:sciomac resume                    # Resume interrupted research session
/oh-my-agent-connector:sciomac list                      # List all research sessions
/oh-my-agent-connector:sciomac report <session-id>       # Generate report for session
```

### Quick Examples

```
/oh-my-agent-connector:sciomac What are the performance characteristics of different sorting algorithms?
/oh-my-agent-connector:sciomac AUTO: Analyze authentication patterns in this codebase
/oh-my-agent-connector:sciomac How does the error handling work across the API layer?
```
...
Task(subagent_type="oh-my-agent-connector:scientist", model="haiku", prompt="[RESEARCH_STAGE:1] Investigate...")

// Stage 2 - Standard analysis
Task(subagent_type="oh-my-agent-connector:scientist", model="sonnet", prompt="[RESEARCH_STAGE:2] Analyze...")

// Stage 3 - Complex reasoning
Task(subagent_type="oh-my-agent-connector:scientist", model="opus", prompt="[RESEARCH_STAGE:3] Deep analysis of...")
```
...
Task(subagent_type="oh-my-agent-connector:scientist", model="sonnet", prompt="
...
4. **Cancellation:** `/oh-my-agent-connector:cancel` or "stop", "cancel"

### AUTO Mode Example

```
/oh-my-agent-connector:sciomac AUTO: Comprehensive security analysis of the authentication system
...
Task(subagent_type="oh-my-agent-connector:scientist", model="haiku", prompt="[STAGE:1] Analyze src/api/...")
Task(subagent_type="oh-my-agent-connector:scientist", model="haiku", prompt="[STAGE:2] Analyze src/utils/...")
Task(subagent_type="oh-my-agent-connector:scientist", model="haiku", prompt="[STAGE:3] Analyze src/components/...")
```
...
Task(subagent_type="oh-my-agent-connector:scientist", model="sonnet", prompt="[HYPOTHESIS:A] Test if caching improves...")
Task(subagent_type="oh-my-agent-connector:scientist", model="sonnet", prompt="[HYPOTHESIS:B] Test if batching reduces...")
Task(subagent_type="oh-my-agent-connector:scientist", model="sonnet", prompt="[HYPOTHESIS:C] Test if lazy loading helps...")
```
...
Task(subagent_type="oh-my-agent-connector:scientist", model="opus", prompt="
[CROSS_VALIDATION]
...
| Command | Action |
|---------|--------|
| `/oh-my-agent-connector:sciomac status` | Show current session progress |
| `/oh-my-agent-connector:sciomac resume` | Resume most recent interrupted session |
| `/oh-my-agent-connector:sciomac resume <session-id>` | Resume specific session |
| `/oh-my-agent-connector:sciomac list` | List all sessions with status |
| `/oh-my-agent-connector:sciomac report <session-id>` | Generate/regenerate report |
| `/oh-my-agent-connector:sciomac cancel` | Cancel current session (preserves state) |
...
Progress is preserved in `.omac/research/{session-id}/` for resume.
