...
```bash
grep -q "oh-my-agent-connector" "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/settings.json" && echo "Plugin verified" || echo "Plugin NOT found - run: claude /install-plugin oh-my-agent-connector"
```
...
```
/oh-my-agent-connector:mcp-setup
```
...
#### 3.3.3: Configure Team Defaults in omac-config
...
Store the team configuration in `~/.claude/.omac-config.json`:
...
Or by running `/oh-my-agent-connector:omac-setup --force` and choosing to enable teams.

## Save Progress

```bash
CONFIG_TYPE=$(jq -r '.configType // "unknown"' ".omac/state/setup-state.json" 2>/dev/null || echo "unknown")
bash "${OMAC_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/scripts/setup-progress.sh" save 6 "$CONFIG_TYPE"
```
