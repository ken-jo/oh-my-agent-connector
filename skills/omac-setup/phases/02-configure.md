...
This will:
1. Install the HUD wrapper script to `~/.claude/hud/omac-hud.mjs`
2. Configure `statusLine` in `~/.claude/settings.json`
3. Report status and prompt to restart if needed

After HUD setup completes, save progress:
```bash
CONFIG_TYPE=$(jq -r '.configType // "unknown"' ".omac/state/setup-state.json" 2>/dev/null || echo "unknown")
bash "${OMAC_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/scripts/setup-progress.sh" save 3 "$CONFIG_TYPE"
```

## Step 2.2: Repair Stale Plugin Cache References

After a marketplace update, Claude Code may still have old OMAC cache paths in the running session or plugin registry. Repair those references before any cache cleanup so setup does not repeatedly emit stale plugin directory errors.
...
const b=p.join(d,'plugins','cache','omac','oh-my-agent-connector');
...
// Try .omac-version.json second
if(v==='')try{const j=JSON.parse(f.readFileSync('.omac-version.json','utf-8'));v=j.version||''}catch{}
// Try CLAUDE.md header third
if(v==='')for(const c of['.claude/CLAUDE.md',p.join(d,'CLAUDE.md')]){try{const m=f.readFileSync(c,'utf-8').match(/^# oh-my-agent-connector.*?(v?\d+\.\d+\.\d+)/m);if(m){v=m[1].replace(/^v/,'');break}}catch{}}
console.log('Installed:',v||'(not found)');
"

# Check npm for latest version
LATEST_VERSION=$(npm view oh-my-agent-connector version 2>/dev/null)
...
if [ -n "$INSTALLED_VERSION" ] && [ -n "$LATEST_VERSION" ]; then
  if [ "$INSTALLED_VERSION" != "$LATEST_VERSION" ]; then
    ...
    echo ""
    echo "To update, run: claude /install-plugin oh-my-agent-connector"
  else
...
Store the preference in `~/.claude/.omac-config.json`:
...
## Step 2.5: Install OMAC CLI Tool

The OMAC CLI (`omac` command) provides standalone helper commands such as `omac hud`, `omac teleport`, and `omac team ...`.

First, check if the CLI is already installed:

```bash
if command -v omac &>/dev/null; then
  OMAC_CLI_VERSION=$(omac --version 2>/dev/null | head -1 || echo "installed")
  echo "OMAC CLI already installed: $OMAC_CLI_VERSION"
  OMAC_CLI_INSTALLED="true"
else
  OMAC_CLI_INSTALLED="false"
fi
```

If `OMAC_CLI_INSTALLED` is `"true"`, skip the rest of this step.

If `OMAC_CLI_INSTALLED` is `"false"`, use AskUserQuestion:

**Question:** "Would you like to install the OMAC CLI globally for standalone helper commands? (`omac`, `omac hud`, `omac teleport`)"

**Options:**
1. **Yes (Recommended)** - Install `oh-my-agent-connector` via `npm install -g`
2. **No - Skip** - Skip installation (can install manually later with `npm install -g oh-my-agent-connector`)

If user chooses **Yes**:

```bash
if ! command -v npm &>/dev/null; then
  echo "WARNING: npm not found. Cannot install OMAC CLI automatically."
  echo "Install Node.js/npm first, then run: npm install -g oh-my-agent-connector"
else
  if npm install -g oh-my-agent-connector 2>&1; then
    echo "OMAC CLI installed successfully."
    if command -v omac &>/dev/null; then
      OMAC_CLI_VERSION=$(omac --version 2>/dev/null | head -1 || echo "installed")
      echo "Verified: omac $OMAC_CLI_VERSION"
    else
      echo "Installed but 'omac' not on PATH. You may need to restart your shell."
    fi
  else
    echo "WARNING: Failed to install OMAC CLI (permission issue or network error)."
    echo "You can install manually later: npm install -g oh-my-agent-connector"
    echo "Or with sudo: sudo npm install -g oh-my-agent-connector"
  fi
fi
```
...
```bash
CONFIG_TYPE=$(jq -r '.configType // "unknown"' ".omac/state/setup-state.json" 2>/dev/null || echo "unknown")
bash "${OMAC_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/scripts/setup-progress.sh" save 4 "$CONFIG_TYPE"
```
