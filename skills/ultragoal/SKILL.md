---
name: ultragoal
description: Durable multi-goal workflow that persists plan/ledger artifacts under .omac/ultragoal and prints Claude /goal handoff text for the active session
...
- The user wants the assistant to literally invoke `/goal` itself from the shell — that is not possible; `omac ultragoal` only writes artifacts and prints handoff text
- The user wants a planning-only artifact with no execution loop — use `plan` instead
</Do_Not_Use_When>

<Why_This_Exists>
Claude Code `/goal` is a session-scoped Stop hook: it blocks the session from stopping until a condition holds, and auto-clears on success. That is a great single-session execution primitive, but it loses state across sessions and does not by itself enforce a final review gate. `omac ultragoal` adds a durable plan, ledger, and gating layer so a long multi-step initiative can survive session restarts, fresh worktrees, and review iterations while still leveraging Claude `/goal` to keep the active agent focused.
</Why_This_Exists>
...
1. Create a plan from a brief:
   ```
   omac ultragoal create-goals --brief-file plan.md
   ```
   Or with explicit stories:
   ```
   omac ultragoal create-goals --brief "ship the migration" \
   ...
   **Multi-repo workspaces / parallel sessions:** when several Claude sessions
   ...
   `.omac/ultragoal/plans/{planId}/` instead of the shared single-plan path.
   ...
   Use `omac ultragoal list-plans` to enumerate available planIds when needed.

2. Start (or resume) the next story:
   ```
   omac ultragoal complete-goals
   ```
...
3. Checkpoint a story:
   ```
   omac ultragoal checkpoint --goal-id G001-... --status complete \
...
4. If the final review is not clean, do NOT mark complete. Record blockers:
   ```
   omac ultragoal record-review-blockers --goal-id G00X-... \
...
5. Inspect state at any time:
   ```
   omac ultragoal status
   ```

</How_To_Use>

<Important_Limitations>
- The shell cannot invoke or mutate Claude Code `/goal` state. `omac ultragoal` only persists durable artifacts and prints instructions that the active Claude agent reads and acts on in-session.
- Snapshots passed via `--claude-goal-json` are model-supplied proof of the active `/goal` state; OMAC validates them for textual consistency with the plan's expected objective and ledger event, but it cannot independently observe Claude `/goal` state.
