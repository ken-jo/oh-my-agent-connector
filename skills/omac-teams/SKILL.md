...
`/omac-teams` is a legacy compatibility skill for the CLI-first runtime: use `omac team ...` commands (not deprecated MCP runtime tools).

## Usage

```bash
/oh-my-agent-connector:omac-teams N:claude "task description"
/oh-my-agent-connector:omac-teams N:codex "task description"
/oh-my-agent-connector:omac-teams N:gemini "task description"
```
...
```bash
/omac-teams 2:claude "implement auth module with tests"
/omac-teams 2:codex "review the auth module for security issues"
/omac-teams 3:gemini "redesign UI components for accessibility"
```
...
- If `$TMUX` is set, `omac team` can reuse the current tmux window/panes directly.
- If `$TMUX` is empty but `CMUX_SURFACE_ID` is set, report that the user is running inside **cmux**. Do **not** say tmux is missing or that they are "not inside tmux"; `omac team` will create **native cmux splits** for workers.
- If neither `$TMUX` nor `CMUX_SURFACE_ID` is set, report that the user is in a **plain terminal**. `omac team` can still launch a **detached tmux session**, but if they specifically want in-place pane/window topology they should start from a classic tmux session first.
...
- Reject unsupported agent types up front. `/omac-teams` only supports **`claude`**, **`codex`**, and **`gemini`**.
- If the user asks for an unsupported type such as `expert`, explain that `/omac-teams` launches external CLI workers only.
- For native Claude Code team agents/roles, direct them to **`/oh-my-agent-connector:team`** instead.
...
`omac team` launches all workers with one shared working directory. For single-repo
...
- If the task references a plan artifact under one repo (for example
  `tool/.omac/plans/task-1200-gwd-gifs.md`) and target paths in sibling repos
...
- Do not anchor the launch cwd to only the repo containing `.omac/plans/...` when
...
- If no safe shared workspace root can be identified, do not launch `/omac-teams`.
...
```bash
omac team <N>:<claude|codex|gemini> "<task>"
```
...
```bash
omac team <N>:<claude|codex|gemini> "<task with absolute plan path and explicit repo paths>" --cwd <workspace-root>
```
...
omac team api list-tasks --input '{"team_name":"<team-name>"}' --json
```

Use `omac team api ...` for task claiming, task transitions, mailbox delivery, and worker state updates.

### Phase 5: Shutdown (only when needed)

```bash
omac team shutdown <team-name>
...
If encountered, switch to `omac team ...` CLI commands.

## Error Reference

| Error                        | Cause                               | Fix                                                                                 |
| ---------------------------- | ----------------------------------- | ----------------------------------------------------------------------------------- |
| `not inside tmux`            | Requested in-place pane topology from a non-tmux surface | Start tmux and rerun, or let `omac team` use its detached-session fallback           |
| `cmux surface detected`      | Running inside cmux without `$TMUX` | Use the normal `omac team ...` flow; OMAC will create native cmux worker splits      |
| `Unsupported agent type`     | Requested agent is not claude/codex/gemini | Use `claude`, `codex`, or `gemini`; for native Claude Code agents use `/oh-my-agent-connector:team` |
| `codex: command not found`   | Codex CLI not installed             | `npm install -g @openai/codex`                                                      |
| `gemini: command not found`  | Gemini CLI not installed            | `npm install -g @google/gemini-cli`                                                 |
| `Team <name> is not running` | stale or missing runtime state      | `omac team status <team-name>` then `omac team shutdown <team-name> --force` if stale |
| `status: failed`             | Workers exited with incomplete work | inspect runtime output, narrow scope, rerun                                         |

## Relationship to `/team`

| Aspect       | `/team`                                   | `/omac-teams`                                         |
| ------------ | ----------------------------------------- | ---------------------------------------------------- |
| Worker type  | Claude Code native team agents            | claude / codex / gemini CLI processes in tmux        |
| Invocation   | `TeamCreate` / `Task` / `SendMessage`     | `omac team [N:agent]` + `status` + `shutdown` + `api` |
