---
name: omac-reference
description: OMAC agent catalog, available tools, team pipeline routing, commit protocol, and skills registry. Auto-loads when delegating to agents, using OMAC tools, orchestrating teams, making commits, or invoking skills.
user-invocable: false
---

# OMAC Reference

Use this built-in reference when you need detailed OMAC catalog information that does not need to live in every `CLAUDE.md` session.
...
- `omac team N:codex|gemini "..."`
- `omac ask <claude|codex|gemini>`
- `/ccg`
...
Invoke built-in workflows via `/oh-my-agent-connector:<name>`.
...
- `omac-plan` — planning workflow and `/plan`-safe alias
- `ralplan` — consensus planning workflow
- `sciomac` — science/research workflow
...
### Utility skills
- `ask`, `cancel`, `note`, `skillify`, `learner` (deprecated alias), `omac-setup`, `mcp-setup`, `hud`, `omac-doctor`, `trace`, `release`, `project-session-manager`, `skill`, `writer-memory`, `configure-notifications`
...
### Example
```text
feat(docs): reduce always-loaded OMAC instruction footprint

Move reference-only orchestration content into a native Claude skill so
session-start guidance stays small while detailed OMAC reference remains available.
