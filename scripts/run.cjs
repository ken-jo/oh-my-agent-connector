...
'use strict';
/**
 * OMAC Cross-platform hook runner (run.cjs)
 *
...
function resolveTarget(targetPath) {
  ...
  // CLAUDE_PLUGIN_ROOT is e.g. ~/.claude/plugins/cache/omac/oh-my-agent-connector/4.2.14
  // We look one level up for sibling version directories.
  try {
    const pluginRoot = process.env.CLAUDE_PLUGIN_ROOT;
    if (!pluginRoot) return null;

    const cacheBase = dirname(pluginRoot);          // .../oh-my-agent-connector/
