...
STATE_FILE=".omac/state/setup-state.json"
CONFIG_DIR="$(resolve_claude_config_dir)"
CONFIG_FILE="$CONFIG_DIR/.omac-config.json"
...
cmd_complete() {
  ...
  # invoked inside omac-setup). Without this, the stop hook blocks with "skill still
  ...
  if [ -n "$sid" ]; then
    ...
    if [[ "$sid" =~ ^[a-zA-Z0-9][a-zA-Z0-9_-]{0,255}$ ]]; then
      rm -f ".omac/state/sessions/${sid}/skill-active-state.json" 2>/dev/null || true
    fi
  else
    # No session ID: fall back to cleaning stale files only (>30min, matching heavy TTL)
    find .omac/state -name "skill-active-state.json" -mmin +30 -delete 2>/dev/null || true
  fi
